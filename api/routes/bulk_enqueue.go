...
func BulkEnqueueRequest(cfg *config.Config, requestQueue queue.RequestQueue) func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		for i := 0; i < len(enqueueMsg); i++ {
			body, err := json.Marshal(enqueueMsg[0])
			if err != nil {
				handleErrorType(w, errors.Wrap(err, "failed to unmarshal enqueue request body"), http.StatusBadRequest, cfg.Logger)
				return
			}
			...
			paramHash := xxhash.Checksum32(enqueueMsg[i].RequestData)

			// Relevant info to run the request downstream
			keyed := pipeline.KeyedEnqueueRequestData{
				EnqueueRequestData: enqueueMsg[i],
